...
export default class DetaliuPaste extends Component {
    constructor(props) {
        super(props)
        this.state = {
            numarProduse: 4,
            ...
            spagetti: false,
            penne: true,
            tipPaste: this.props.route.params.paste.optionals.length > 0 ? this.props.route.params.paste.optionals[0].optionalsContent[0] : null,
            topinguriAdaugate: [],
            toateSosurile: [],
            toateTopingurile: [],
            ingredienteCePotFiScoase: [],
            paste: this.props.route.params.paste,
        }
    }
    componentDidMount(){
        console.log("component mounted", this.props.route.params.paste)
        this.setState({paste: this.props.route.params.paste})
        this.getPasteDetails()
    }
    getPasteDetails(){
        api.get('/getDetaliuPaste', {id: this.state.paste})
        .then(res => {
            if(!res.data.success)
                ...
                if(res.data.ingredienteBlocate == null){
                    ingrediente = this.state.paste.removableToppings
                }else{
                    this.state.paste.removableToppings.map((item)=>{
                ...
                if(res.data.toppinguriBlocate == null){
                    toppings = this.state.paste.removableToppings
                }else{
    ...
    setPaste() {
        if(this.state.penne){
            this.state.spagetti=true
            this.state.penne=false
            this.state.tipPaste= this.state.paste.optionals[0].optionalsContent[1]
        }else{
            this.state.spagetti=false
            this.state.penne=true
            this.state.tipPaste= this.state.paste.optionals[0].optionalsContent[0]
        }
        this.forceUpdate()
    }
    ...
    addToCart(){
        var options= _.cloneDeep(this.state.paste)
        options['removed']=this.state.removed
        options['extra']=this.state.topinguriAdaugate
        if (this.state.tipPaste) options['tipPaste']=this.state.tipPaste
        var item = {
            id: this.state.paste.productId.toString(),
            name: this.state.paste.name,
            price: this.state.paste.price+this.state.totalPriceToppings,
    ...
    render() {
        ...
        smallPizzaPressedElement.push(
                <TouchableOpacity activeOpacity = {1}  onPress = {()=>this.setPaste()}  activeOpacity = {0.7} style = {[styles.dimensiunePizza, this.state.penne ? {borderColor:'#FFD101'} : {borderColor:'#999999'}]}>
                    {this.state.penne ? <PennePressed width = {46} height = {46}></PennePressed> : <Penne width = {46} height = {46}></Penne>}
                     
                     <Text style = {[styles.dimensiunePizzaText,this.state.penne ? {color:'#FFD101'} : {color:'#999999'}]}>Penne</Text>
                 </TouchableOpacity>
        )
        var bigPizzaPressedElement = [];
        bigPizzaPressedElement.push(
            <TouchableOpacity activeOpacity = {1}  onPress = {()=>this.setPaste()}  activeOpacity = {0.7} style = {[styles.dimensiunePizza, this.state.spagetti ? {borderColor:'#FFD101'} : {borderColor:'#999999'}]}>
                {this.state.spagetti ? <SpaghettiPressed width = {63} height = {43}></SpaghettiPressed> : <Spaghetti width = {63} height = {43}></Spaghetti>}
                
                <Text style = {[styles.dimensiunePizzaText,this.state.spagetti ? {color:'#FFD101'} : {color:'#999999'}]}>Spaghetti</Text>
            </TouchableOpacity>
        )
            ...
            <View style={{ width: '100%', height: '100%', backgroundColor: 'black' }}>
                ...
                <View style={[styles.container, { paddingBottom: 70 }]}>
                    <ScrollView style={styles.contentScrollView}>
                        ...
                        <Image style={styles.imageProdus} source={{uri: api.img("width:400",this.state.paste.image)}} />
                        </View>
                        <Text style={styles.detaliuProdusTitle}>{this.state.paste.name}</Text>
                        ...
                        {this.state.paste.optionals.length > 0 ? (
                            <>
                            <Text style={styles.ingredienteText}>Alege tipul de paste:</Text>
                        ...
                        <View style={styles.numberProducsContainer}>
                            ...
                            <View style={[styles.ofertaPretContainer, {marginLeft:10}]}>
                                <Text style={[styles.ofertaPret, { fontSize: 31 }]}>{(this.state.paste.price+this.state.totalPriceToppings)*this.state.countProduse}</Text>
